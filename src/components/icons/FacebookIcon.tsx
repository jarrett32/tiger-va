import Link from "next/link";
import React from "react";

const Facebook = (): JSX.Element => (
  <Link href="" legacyBehavior>
    <a
      className="hover:bg-facebook group mb-2 flex h-10 w-10 flex-shrink-0 items-center justify-start rounded-full transition"
      target="_blank"
    >
      <svg
        className="fill-current text-black transition group-hover:text-white"
        width="12"
        height="19"
        viewBox="0 0 12 19"
        fill="none"
        xmlns="http://www.w3.org/2000/svg"
      >
        <title>Tiger VA Facebook</title>
        <path d="M10.4678 10.6875L11.001 7.24895H7.6669V5.01756C7.6669 4.07684 8.13265 3.15986 9.6259 3.15986H11.1416V0.232305C11.1416 0.232305 9.76615 0 8.45102 0C5.70527 0 3.91052 1.64691 3.91052 4.62828V7.24895H0.858398V10.6875H3.91052V19H7.6669V10.6875H10.4678Z" />
      </svg>
    </a>
  </Link>
);

export default Facebook;
